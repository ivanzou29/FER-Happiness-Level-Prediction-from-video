...
def plot_result_comparison(model_list, batch_size, loss_function, column):
    ...
    for model in model_list:
        ...
        epoch_list_new = np.linspace(epoch_list.min(), epoch_list.max(), 50)
        metric_list_new = spline(epoch_list, metric_list, epoch_list_new)
        plt.plot(epoch_list, metric_list, style)
    
    plt.legend(model_list, bbox_to_anchor=(1.05, 0), loc=3, borderaxespad=0)
    plt.title('batch size = ' + str(batch_size) + ', loss function = ' + loss_function)
    plt.xlabel('epoch')
    plt.ylabel(column_name)
    plt.savefig('batch size = ' + str(batch_size) + ', loss function = ' + loss_function + ', ' + column_name + '.png', bbox_inches = 'tight')
