...
import torch
import data_preprocessing_face_two_eyes_mouth
...
parser.add_argument('-r', '--rnnlayer', type=int, default=1, help='rnn layers')
...
HISTORY_DIR = "vgg_reduced_model_with_RNN_face_two_eyes_mouth_history_evenly_sampled/"
...
class CompoundModel(nn.Module):
    def __init__(self):
        ...
        self.cnn_3 = CNN(my_vgg_arch)
        self.rnn = nn.LSTM(input_size=4 * FC_LAYER_OUTPUT_NUM, hidden_size=FC_LAYER_OUTPUT_NUM * 10, num_layers=LSTM_LAYERS, batch_first=True)
    ...
    def forward(self, x):
        ...
        face_in = face.view(-1, C, H, W)
        region1_in = face.view(-1, C, H, W)
        region2_in = face.view(-1, C, H, W)
        region3_in = face.view(-1, C, H, W)
        ...
        face_out = self.cnn_0(face_in)
        region1_out = self.cnn_1(region1_in)
        region2_out = self.cnn_2(region2_in)
        region3_out = self.cnn_3(region3_in)
        ...
        r_in = torch.cat((face_out, region1_out, region2_out, region3_out), 1)
        ...
        r_in = r_in.view(-1, TIME_STEP, 4 * FC_LAYER_OUTPUT_NUM)
...
if __name__ == '__main__':
    ...
    my_data, my_label = data_preprocessing_face_two_eyes_mouth.get_all_data(TRAIN_VAL_DIR, TIME_STEP)
    train_data, train_label, val_data, val_label = train_val_split(my_data, my_label)
    del my_data
    del my_label

    # get the testing data
    test_data, test_label = data_preprocessing_face_two_eyes_mouth.get_all_data(TEST_DIR, TIME_STEP)
