...
import torch
import data_preprocessing_face_mouth
...
class CompoundModel(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.LSTM(input_size= 20, hidden_size=10, num_layers=1, batch_first=True)
    ...
    def forward(self, x):
        ...
        r_in = torch.cat((face_out, mouth_out), 1)
...
if __name__ == '__main__':
    ...
    my_data, my_label = data_preprocessing_face_mouth.get_all_data(main_dir)
    val_data, val_label = data_preprocessing_face_mouth.get_all_data(val_dir)
    ...
    f = open('vgg_reduced_face_mouth_model_training_history_BatchSize=' + str(BATCH_SIZE) + '_Epoch=' + str(EPOCH) + '.txt', 'w+')
    ...
    name = 'vgg_reduced_face_mouth_model_training_history_BatchSize=' + str(BATCH_SIZE) + '_Epoch=' + str(EPOCH) 
    plt.title(name)
