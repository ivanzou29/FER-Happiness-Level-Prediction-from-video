...
import torch
import data_preprocessing_face_eye_nose
...
history_dir = 'vgg_reduced_face_eye_nose_training_history_RMSE/'
...
class CompoundModel(nn.Module):
    ...
    def forward(self, x):
        ...
        nose_in = nose.view(batch_size * time_step, C, H, W)
        ...
        nose_out = self.cnn_nose(nose_in)

        del face_in
        del left_eye_in
        del nose_in

        r_in = torch.cat((face_out, left_eye_out, nose_out), 1)
...
if __name__ == '__main__':
    ...
    my_data, my_label = data_preprocessing_face_eye_nose.get_all_data(main_dir)
    val_data, val_label = data_preprocessing_face_eye_nose.get_all_data(val_dir)
    ...
    f = open(history_dir + 'vgg_reduced_face_eye_nose_model_training_history_BatchSize=' + str(BATCH_SIZE) + '_Epoch=' + str(EPOCH) + '.txt', 'w+')
    ...
    name = 'vgg_reduced_face_eye_nose_model_training_history_BatchSize=' + str(BATCH_SIZE) + '_Epoch=' + str(EPOCH) 
    plt.title(name)
